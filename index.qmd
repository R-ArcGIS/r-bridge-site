...
ArcGIS Location Services are provided as REST API endpoints. **`arcgis`** enables seamless interaction with these services using R native objects. Examples of ArcGIS Location Services include:

- [Data hosting](https://developers.arcgis.com/documentation/mapping-apis-and-services/data-hosting/)
- [Geocoding](https://developers.arcgis.com/documentation/mapping-apis-and-services/geocoding/services/geocoding-service/) (**`arcgis`** support in development)
- [Routing](https://developers.arcgis.com/documentation/mapping-apis-and-services/routing/services/routing-service/) (**`arcgis`** support in development)

# Interact with ArcGIS Pro

`{arcgisbinding}`, the first package created for the R-ArcGIS Bridge, is designed to bind the powers of R and ArcGIS Pro. Using `{arcgisbinding}` you can read and write local data sources, call [ArcPy functions](https://pro.arcgis.com/en/pro-app/latest/arcpy/get-started/what-is-arcpy-.htm) from R, take advantage of ArcGIS Pro's [projection engine](https://github.com/Esri/projection-engine-db-doc), or create [geoprocessing script tools that use R](geoprocessing-tools.qmd). 
...
Move data between R and ArcGIS Pro. Use ArcPy from R. Build R-driven geoprocessing tools.
:::
