...
- filter the Feature Layer query by column
- filter the Feature Layer query by row
- use `dplyr` for filtering
...
Clicking this will bring you to the Feature Service. Inside of a Feature Server there may be many layers or tables that we can use. In this case, there is only one layer. Click the hyperlinked **USA Major Cities**. 
...
With this `FeatureLayer` object, we can read data from the service into R! 
