...
Much like a python geoprocessing (GP) [script tool](https://pro.arcgis.com/en/pro-app/latest/help/analysis/geoprocessing/basics/create-a-python-script-tool.htm), GP script tools and toolboxes can be created utilizing the power of R. 
...
Values can be extracted from the `in_params` and `out_params` lists by the name or position of the parameter. We highly recommended using name-based indexing rather than position-based indexing for clarity.

<!-- 
Martha: Since other/previous arcgisbinding documentation (and Python script tool/pyt documentation) uses position-based indexing, I think we should mention it but keep our recommendation to use the names.
--> 
...
Below is an example of a function to parse a parameter named `date`:
...
that parameter will appear in the Geoprocessing pane. Each ArcGIS Pro parameter type can be represented by a basic scalar R type: `integer`, `double`, `character`, `logical`, or 
`NULL`.
...
<!-- 
Martha: Copied wording from here: https://pro.arcgis.com/en/pro-app/latest/arcpy/classes/parameter.htm
--> 
...
be represented in R as a list of scalars. Take the below input, for example:
...
<!-- 
Martha: Tried to consolidate the info above without losing the emphasis.
--> 
...
:::{.callout-note}
As of Nov 2023, support for `{terra}` is in development.
:::
...
When sharing R-based GP tools with other users, they may not have the packages that the script tool needs to execute code. In this case, required packages should be automatically installed the first time the script tool is executed.
...
This function uses `requireNamespace()` which attempts to load the provided package. If it succeeds, it returns `TRUE`, if not it returns `FALSE`. The function checks to see if `FALSE` is returned and if so, installs the package and prints an informative message. 

For example, if the script requires the package [`{spdep}`](https://r-spatial.github.io/spdep/) and it is not installed, the function will print the message and install it.
...
<!-- 
Martha: Happy to take a stab at the above two sections once I get through a 
first look at all the pages!
--> 