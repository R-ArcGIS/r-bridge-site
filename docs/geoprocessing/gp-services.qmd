...
We interact with geoprocessing services through a [GP job](https://developers.arcgis.com/rest/services-reference/enterprise/gp-job/) represented by the `arc_gp_job` R6 class. 

A new geoprocessing job can be created with `arc_gp_job$new()` with the following structure: 
...
The geoprocessing framework has [service data types](https://developers.arcgis.com/rest/services-reference/enterprise/gp-data-types/) that can be provided to a geoprocessing job.

Parameters must be provided as a named list where the name is the exact name of the parameter and the value is a string of the json that must be provided as the parameter. Most geoprocessing parameters can be created and parsed using arcgisutils. 
...
Most geoprocessing services require a **feature set** which can be created with `as_esri_featureset()`.
...
An `arc_gp_job` has a `result_fn` which is 


For an example we will work with the [Trace Downstream](https://developers.arcgis.com/rest/elevation-analysis/trace-downstream/) elevation service. 
